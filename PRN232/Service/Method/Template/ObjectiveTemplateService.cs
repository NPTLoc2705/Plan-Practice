...
namespace Service.Method.Template
{
    public class ObjectiveTemplateService : IObjectiveTemplateService
    {
        ...
        private ObjectiveTemplate MapToEntity(ObjectiveTemplateRequest request, int userId, int? id = null)
        {
            ...
            {
                throw new ArgumentException("Objective name is required and cannot be empty.");
            }

            var trimmedName = request.Name.Trim();
            if (trimmedName.Length > 200)
            {
                throw new ArgumentException("Objective name must be 200 characters or less.");
            }

            // Validate content
            if (string.IsNullOrWhiteSpace(request.Content))
            {
                throw new ArgumentException("Objective content is required and cannot be empty.");
            }

            var trimmedContent = request.Content.Trim();
            if (trimmedContent.Length > 2000)
            {
                throw new ArgumentException("Objective content must be 2000 characters or less.");
            }
