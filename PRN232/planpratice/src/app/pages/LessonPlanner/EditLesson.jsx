  ...
  useEffect(() => {
    const fetchAllDataForEdit = async () => {
      ...
      try {
        ...
        setInteractionPatterns(patternsData);
        setAttitudeTemplates(attitudesData);
        setObjectiveTemplates(objectivesData);
        setPreparationTemplates(preparationsData);
        setSkillTemplates(skillsData);
        setLanguageFocusTypes(languageTypesData);
        ...
        setSelectedMethod(lessonToEdit.methodTemplateId || '');
        ...
        // Populate selections
        setSelectedObjectives(lessonToEdit.objectives?.map(o => o.objectiveTemplateId) || []);
        setSelectedSkills(lessonToEdit.skills?.map(s => s.skillTemplateId) || []);
        setSelectedAttitudes(lessonToEdit.attitudes?.map(a => a.attitudeTemplateId) || []);
        // Map languageFocusTypeId from backend to typeId for frontend
        setLanguageFocus(lessonToEdit.languageFocusItems?.length > 0 
          ? lessonToEdit.languageFocusItems.map(lf => ({ 
              typeId: lf.languageFocusTypeId, 
              content: lf.content || '' 
            })) 
          : [{ typeId: null, content: '' }]);

        // Reconstruct preparations with name from template
        const reconstructedPreparations = lessonToEdit.preparations?.map(prep => {
        ...
        // Reconstruct activities structure for the UI
        const reconstructedActivities = lessonToEdit.activityStages?.map(stage => ({
      ...
      activityStages: activities.map((stage, stageIndex) => ({
        ...
        activityItems: stage.subActivities.map((sub, subIndex) => {
          ...
          return {
            ...
            interactionPatternId: sub.interactionPatternId ? parseInt(sub.interactionPatternId) : null,
            activityTemplateId: sub.activityTemplateId ? parseInt(sub.activityTemplateId) : null,
  ...
  const generateLessonContent = () => {
    ...
    if (selectedSkills.length > 0) {
      ...
      selectedSkills.forEach(skillId => {
        ...
        if (skill) {
          html += `<li><strong>${skill.name || skill.skillType}:</strong> ${skill.description}</li>`;
        }
      });
      html += `</ul>`;
    }
